...
function DropdownContactoSos({ contacto }) {
  ...
  const modificar_vehiculo =  (e) => {
    ...
    e.preventDefault();
    axios.put(
      `https://api-ucabrides-v2-7913fcd58355.herokuapp.com/api/contactosos/` + contacto._id,
      { nombre: nombre, telefono: telefono }, {headers: {
  ...
  const eliminar_vehiculo =  () => {
    setOpen_modal(false);
    axios.delete(
      `https://api-ucabrides-v2-7913fcd58355.herokuapp.com/api/contactosos/` + contacto._id, {headers: {
        Authorization: `Bearer ${localStorage.getItem("access_token")}`
      }}
    ).then(()=>{
