...
export default function ActivarRuta({
  ...
  const handleActivar = ()=>{
    if(asientos>4){
      ...
      setOpen(false);
      axios.post(
        `https://api-ucabrides-v2-7913fcd58355.herokuapp.com/api/crear_orden`,
        { vehicle_id:vehiculoSeleccionado,route_id:ruta_id,seats:asientos,hour:hora},{headers: {
        ...
        enqueueSnackbar(response.data.error,{ variant: "success" })
          ...
          localStorage.setItem("ucabrides_orden_ruta_id",response.data._id)
          refresh();
