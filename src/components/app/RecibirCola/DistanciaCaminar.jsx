...
function DistanciaCaminar() {
  ...
  const handlecambiar = async () => {
    if (distancia>0) {
      try {
        ...
        const res = await axios.post(`https://api-ucabrides-v2-7913fcd58355.herokuapp.com/api/cambiar_distancia_caminar`, {
          distancia: distancia,
        }, {
            headers: {
  ...
  function obtener_distancia() {
    // CANTIDAD EN MT QUE EL USUARIO ESTA DISPUESTO A CAMINAR
      axios.get("https://api-ucabrides-v2-7913fcd58355.herokuapp.com/api/distancia_dispuesto_caminar", {headers: {
        Authorization: `Bearer ${localStorage.getItem("access_token")}`
      }}).then((response) => {
